...
if (isset($_SESSION['id_proker'])) {
    ...
    if (empty($proker)) {
        header("Location:index.php?include=edit-proker-bpi&data=" . $id_proker . "&notif=editprokerkosong");
        } 
        else if (empty($deskripsi_proker)) {
        header("Location:index.php?include=edit-proker-bpi&data=" . $id_proker . "&notif=editdeskripsikosong");
        } 
        else if (empty($departemen)) {
        header("Location:index.php?include=edit-proker-bpi&data=" . $id_proker . "&notif=departemenkosong");
        } 
        else {
    ...
    WHERE `id_proker`='$id_proker'";
        ...
        header("Location:index.php?include=bpi&notifproker=editprokerberhasil");
        } 
